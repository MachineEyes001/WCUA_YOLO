...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Args:
        ...
        package (str, optional): Python package to use instead of requirements.txt file, i.e. package='WCUA-YOLO'.
    ...
    Example:
        ```python
        from WCUA-YOLO.utils.checks import parse_requirements

        parse_requirements(package="WCUA-YOLO")
        ```
    """
    if package:
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='WCUA-YOLO'
        try:
...
def check_latest_pypi_version(package_name="WCUA-YOLO"):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the WCUA-YOLO package is available on PyPI.
        ...
        try:
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    ...
                    f"Update with 'pip install -U WCUA-YOLO'"
                )
                return True
        except Exception:
            pass
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    ...
    Example:
        ```python
        from WCUA-YOLO.utils.checks import check_requirements

        # Check a requirements.txt file
        check_requirements("path/to/requirements.txt")

        # Check a single package
        check_requirements("WCUA-YOLO>=8.0.0")

        # Check multiple packages
        check_requirements(["numpy", "WCUA-YOLO>=8.0.0"])
        ```
    """
...
def collect_system_info():
    ...
    package_info = {}
    for r in parse_requirements(package="WCUA-YOLO"):
        try:
...
def check_amp(model):
    ...
    Example:
        ...
        from WCUA-YOLO.utils.checks import check_amp
