...
HELP_MSG = """
    ...
    1. Install the WCUA-YOLO package:

        pip install WCUA-YOLO
...
class TQDM(tqdm_original):
    ...
    Examples:
        >>> from WCUA-YOLO.utils import TQDM
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date WCUA-YOLO
            'default.yaml' file.\nPlease update your code with 'pip install -U WCUA-YOLO' and if necessary replace
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    Examples:
        >>> set_logging(name="WCUA-YOLO", verbose=True)
        >>> logger = logging.getLogger("WCUA-YOLO")
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from WCUA-YOLO.utils import ThreadingLocked
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                ...
                f"with your settings or a recent WCUA-YOLO package update. {self.help_msg}"
            )
            self.reset()
...
def vscode_msg(ext="WCUA-YOLO.WCUA-YOLO-snippets") -> str:
