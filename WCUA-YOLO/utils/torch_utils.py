...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    Example:
        ```python
        from pathlib import Path
        from WCUA-YOLO.utils.torch_utils import strip_optimizer
    ...
    Note:
        Use `WCUA-YOLO.nn.torch_safe_load` for missing modules with `x = torch_safe_load(f)[0]`
    """
    try:
...
def profile(input, ops, n=10, device=None, max_num_obj=0):
    ...
    Example:
        ```python
        from WCUA-YOLO.utils.torch_utils import profile
